...
class Net:
    def __init__(self, log, path, name):
        ...
        self._path = path
        self._status = False
        self._model = None  # self._load_from_path_standard()
    ...
    def get_layers(self):
        return self._parameterized.blobs.keys()
            ...
            if len(os.listdir(self._path_to_models)) >= 1:
                for net_name in os.listdir(self._path_to_models):
                    try:
                        ...
                        self._log.fatal("Error loading model (" + net_name + ")")
                        return False
                return True
            else:
                self._log.fatal("Nope models for download!")
        except BaseException as e:
            self._log.debug(str(e.args))
            self._log.fatal("Error loading models!!!")
